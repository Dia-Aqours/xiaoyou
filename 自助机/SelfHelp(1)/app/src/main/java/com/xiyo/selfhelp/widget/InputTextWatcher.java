...
public class InputTextWatcher implements TextWatcher {
    ...
    ManagerDialog.PhoneInputListener phoneInputListener;

    int cursor = 0;// 用来记录输入字符的时候光标的位置
    int before_length;// 用来标注输入某一内容之前的编辑框中的内容的长度

    public InputTextWatcher(EditText text, int limit,
                            Context context, ManagerDialog.PhoneInputListener phoneInputListener) {
        this.limit = limit;
        this.text = text;
...
//        Log.e("此时你已经输入了", "" + s.length());
        ...
        if(after_length < limit){
            if(phoneInputListener != null){
                phoneInputListener.onError("");
            }
        }else if(after_length == limit){
            if(phoneInputListener != null){
                phoneInputListener.onSuccess();
            }
        }if (after_length > limit) {
